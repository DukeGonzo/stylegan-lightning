...
class ModulatedBlock(pl.LightningModule):
    def __init__(self,
        ...
        self.style_conv_up = layers.StyleConvUp(
            latent_size, in_channels, out_channels, filter_size=3)
        self.style_conv = layers.StyleConv(
            latent_size, out_channels, out_channels, filter_size=3)
...
class SynthesisNetwork(pl.LightningModule):
    def __init__(self,
        ...
        self.constant = layers.ConstantLayer(
            in_channels, 4)  # TODO: remove hardcode

        self.style_conv = layers.StyleConv(
            latent_size, in_channels, in_channels, filter_size=3)
        self.to_rgb = layers.ToRgb(latent_size, in_channels)

        for i in range(3, log_res + 1):
            out_channels = self.channels[2 ** i]
            self.blocks.append(ModulatedBlock(
                in_channels, out_channels, latent_size))
    ...
    def make_noise(self,  batch_size: int) -> List[torch.Tensor]:
        ...
        for i, map_size in enumerate(self.channels.keys()):
            ...
            for _ in range(2):
                noise = torch.randn(batch_size, 1, map_size,
                                    map_size, device=self.device)
    ...
    def forward(self,
                ...
                target_resolution: Optional[int] = None) -> torch.Tensor:
        """
        Args:
            latent_vectors (torch.Tensor): latent vectors (after mapping network)
            noise (Optional[List[torch.Tensor]], optional): Spatial noise. Defaults to None.
