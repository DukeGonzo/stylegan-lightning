...
from models.critic import CriticNetwork, MultiResolutionCriticNetwork
...
class GanTask(pl.LightningModule):
    def __init__(self,
        ...
        self.synthesis_net = SynthesisNetwork(
            resolution=resolution, latent_size=latent_size, channel_multiplier=1)

        if use_anycost_gan:
            self.critic_net = MultiResolutionCriticNetwork(
                resolution=resolution, label_size=label_size, channel_multiplier=1)
        else:
    ...
    def forward(self,
        ...
        return self.synthesis_net.forward(w_plus, target_resolution), w_plus
    ...
    def r1_penalty(real_score: torch.Tensor, real_input: torch.Tensor) -> torch.Tensor:
        grad_real, = autograd.grad(outputs=real_score.sum(
        ), inputs=real_input, retain_graph=True, create_graph=True)
    ...
    def ppl_regularization(self, fake_img: torch.Tensor, latents: torch.Tensor, decay: float = 0.01) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        noise = torch.randn_like(
            fake_img, device=self.device) / math.sqrt(fake_img.shape[2] * fake_img.shape[3])
    ...
    def configure_optimizers(self):
        ...
        generator_opt = optim.Adam(chain(self.synthesis_net.parameters(
        ), self.mapping_net.parameters()), lr=0.0025, betas=(0.0, 0.99))
        critic_opt = optim.Adam(
            self.critic_net.parameters(), lr=0.0025, betas=(0.0, 0.99))
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        labels = F.one_hot(
            labels.long(), self.mapping_net.label_size)  # TODO: remove
        ...
        fake_images, latents = self.forward(
            batch_size, labels, target_resolution=sampled_resolution)

        # get optimizers
        # type: ignore  # Pylance bug
        ...
        # DETACH is used to prevent extra computations
        fake_scores_no_gen = self.critic_net.forward(
            fake_images.detach(), labels)
        critic_loss = self.critic_non_saturating_gan_loss(
            fake_scores_no_gen, real_scores)
        ...
        fake_scores = self.critic_net.forward(
            fake_images, labels, return_activations=False)
        ...
        if batch_idx % self.ppl_reg_every == 0:
            path_loss, path_lengths = self.ppl_regularization(
                fake_images, latents)
